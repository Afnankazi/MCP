...
export class CommandParser {
  ...
  parseCommand(input) {
    ...
    const hasGenerateIntent = generateKeywords.some((kw) =>
  ...
  extractFilePath(input) {
    ...
    const extMatch = cleanInput.match(
      /[\w\-]+\.(js|ts|py|java|go|rs|cpp|c|rb|php|jsx|tsx)/i
    );
  ...
  extractCode(input) {
    ...
    const afterCommand = input
      .replace(/detect\s+bugs?\s+in\s+(this\s+)?code[:=]?\s*/gi, "")
      .trim();
